...
def _reverse_diffusion_hcycle(
    ...
    if generator is None:
        traj = torch.randn(batch_size, 7 * seq_len, hidden_dim, device=device)
    else:
    ...
    with torch.no_grad():
        for split_name, batch, _ in eval_loader:
            ...
            if keep_indices is not None:
                keep_tensor = torch.tensor(keep_indices, dtype=torch.long, device=full_inputs.device)
...
def main():
    ...
    if args.eval_after_train:
        ...
        eval_group = dist.group.WORLD if dist.is_initialized() else None
        final_metrics = _run_hcycle_evaluation(
            args,
            model_state_dict=state_dict_cpu,
