#!/usr/bin/env python
"""
Evaluate a hybrid model: use teacher weights for everything except the core,
and use the shared one-step core weights for all TRM cores. Cores are replaced
via streaming_forward replacements, sampling fresh eps ~ N(0, I) per call.

Example:
python TinyRecursiveModels/eval_onestep_hybrid.py \
  --teacher-checkpoint TinyRecursiveModels/checkpoints/trm_sudoku_att_step21700.pt \
  --onestep-checkpoint analysis/core_all_onestep/core_all_one_step_latest.pt \
...
class OneStepCoreModel(torch.nn.Module):
    def __init__(self, seq_len_clean: int, hidden_dim: int, num_layers: int, num_heads: int, expansion: float, rms_norm_eps: float, rope_theta: float):
        ...
        self.out_proj = torch.nn.Linear(total_len, seq_len_clean)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Eval hybrid teacher + one-step core swap")
...
def main(args: argparse.Namespace) -> None:
    ...
    def replacement_fn(x_clean: torch.Tensor, noise: torch.Tensor) -> torch.Tensor:
        # Fresh eps ~ N(0, I); ignore teacher noise to ensure core-only behavior.
    ...
    while total < args.eval_puzzles:
        ...
        logits, _ = teacher.streaming_forward(
            batch=batch,
            steps=args.steps,
            noise_scale=0.0,  # teacher cores disabled; replacements provide outputs
