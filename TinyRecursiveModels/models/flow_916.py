"""
Flow matching model for single TRM layer (seq_len=916).
...
class SingleLayerFlow(nn.Module):
    """
    Flow matching model for a single TRM layer update.
    Uses the same TRM block architecture for consistency.
    
    Predicts the velocity field of a target latent space given:
        - current_latent: [B, 916, 512] - current latent state
        - timestep: [B] - timestep
